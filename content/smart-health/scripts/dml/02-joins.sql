...
--obtener los pacientes (primer nombre,genero,correo)

--INNER JOIN 
-- smart_health.patients : patient_id (PK)
--smart_health.patient_phone: patient_id (FK)
...
----------------------------------------------------------
--Contar medicos sin dirección
--LEFT JOIN 
--smart_health.doctors: doctor_id (PK)
--smarth_health.doctor_addresses: doctor_id (FK)
SELECT
...
--mostrar nombre completo del paciente, el genero, tipo de sangre, direccion, ciudad y departamento 
...
SELECT
    ...
    T2 address_id,
    T3 postal_code,
    T4 municipality_code,
    T5 department_name 

FROM smarth_health.addresses A
INNER JOIN smart_health.patients B
    ON A.patient_id = B.patient_id
